...
$(function () {
  ...
  if (dt_user_table.length) {
    var dt_user = dt_user_table.DataTable({
        ajax: assetsPath + 'json/patient-list.json', // Updated JSON file path
        columns: [
            { data: '' }, // For checkbox or serial number
            { data: 'patient_name' }, // Patient Name
            { data: 'email' }, // Email
            { data: 'phone_number' }, // Phone Number
            { data: 'create_date' }, // Create Date
            { data: 'last_appointment_date' }, // Last Appointment Date
            { data: 'total_appointments' }, // Total Appointments
            { data: 'payment_status' }, // Payment Status
            { data: 'action' } // Actions (Edit/Delete buttons)
        ],
      columnDefs: [
        {
          // For Responsive
...
'<a href="' +
patientView + // Updated variable for patient details view
'" class="text-truncate text-heading"><span class="fw-medium">' +
$patient_name + // Updated to Patient Name
'</span></a>' +
'<small>' +
$email + // Patient Email
'</small>' +
'<small>📞 ' + $phone_number + '</small>' + // Added phone number display
'</div>' +
'</div>';
return $row_output;
      ...
      initComplete: function () {
        ...
        this.api()
          ...
          .every(function () {
            var column = this;
            var select = $(
              '<select id="patientname" class="form-select text-capitalize"><option value=""> Select Name </option></select>'
            )
              .appendTo('.patient_name')
              .on('change', function () {
